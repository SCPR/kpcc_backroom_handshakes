...
class BuildableMixin(object):
    ...
    def get_content(self):
        ...
        if settings.STAGING == True:
            settings.STATIC_URL = "%s%s%s" % (settings.STAGING_PREFIX, settings.DEPLOY_DIR, settings.STATIC_DIR)
            settings.URL_PATH = "%s%s" % (settings.STAGING_PREFIX, settings.DEPLOY_DIR)
        else:
            settings.STATIC_URL = "%s%s%s" % (settings.LIVE_PREFIX, settings.DEPLOY_DIR, settings.STATIC_DIR)
            settings.URL_PATH = "%s%s" % (settings.LIVE_PREFIX, settings.DEPLOY_DIR)
